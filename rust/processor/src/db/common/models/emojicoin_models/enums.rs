...
impl EmojicoinTypeTag {
    pub fn from_type_str(type_str: &str) -> Option<Self> {
        match type_str {
            ...
            str if ARENA_MELEE_EVENT.as_ref().map_or(false, |s| s == str) => Some(Self::ArenaMelee),
            str if ARENA_ENTER_EVENT.as_ref().map_or(false, |s| s == str) => Some(Self::ArenaEnter),
            str if ARENA_EXIT_EVENT.as_ref().map_or(false, |s| s == str) => Some(Self::ArenaExit),
            str if ARENA_SWAP_EVENT.as_ref().map_or(false, |s| s == str) => Some(Self::ArenaSwap),
